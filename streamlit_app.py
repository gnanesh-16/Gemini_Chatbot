...
def convert_chat_to_text(chat_history):
    ...
    for message in chat_history:
        ...
        text += f"{timestamp} {role_to_streamlit(message.role)}: {message.parts[0].text}\n"
    return text
...
for message in st.session_state.chat.history:
    timestamp = message.timestamp.strftime("%Y-%m-%d %H:%M:%S")
    with st.expander(f"{timestamp} {role_to_streamlit(message.role)}"):
        st.markdown(message.parts[0].text)

# Accept user's next message, add to context, resubmit context to Gemini
if prompt := st.text_input("I possess a well of knowledge. What would you like to know?"):
    # Display user's last message
    st.expander(f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} User: {prompt}")

    # Send user entry to Gemini and read the response
    response = st.session_state.chat.send_message(prompt)
    timestamp = response.timestamp.strftime("%Y-%m-%d %H:%M:%S")
    with st.expander(f"{timestamp} Assistant"):
        st.markdown(response.text)
